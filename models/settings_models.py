...
class SettingsModel:
    ...
    def get_all_data(self, table_name):
        ...
        try:
            if table_name == "solicitantes":
                ...
                query = """
                SELECT p.id_producto, p.codigo, p.nombre, m.nombre as marca, c.nombre as categoria, 
    ...
    def get_active_data(self, table_name):
        ...
        try:
            if table_name == "solicitantes":
                ...
                FROM solicitantes s
                LEFT JOIN departamentos d ON s.id_departamento = d.id_departamento
                WHERE s.activo = TRUE AND d.activo = TRUE  # <-- Agregar filtro para departamento activo
                """
            elif table_name == "productos":
                query = """
                SELECT p.id_producto, p.codigo, p.nombre, m.nombre as marca, c.nombre as categoria
                ...
                LEFT JOIN marcas m ON p.id_marca = m.id_marca
                LEFT JOIN categorias c ON p.id_categoria = c.id_categoria
                WHERE p.activo = TRUE AND m.activo = TRUE AND c.activo = TRUE  # <-- Agregar filtros para marca y categoria activas
                """
            elif table_name == "usuarios":
    ...
    def get_related_options(self, table_name):
        ...
        try:
            # Para tablas que terminan con 's' (plural)
            ...
            raise Exception(f"Error al obtener opciones: {str(e)}")
