...
class SupplierController:
    ...
    def show_supplier_form(self, supplier_id=None):
        ...
        try:
            ...
            form_window, entries, save_btn = self.view.show_supplier_form(self.app, supplier_id)
            
            # Configurar valores de combobox
            if "Valoración:" in entries:
                entries["Valoración:"]['values'] = ["1 Estrella", "2 Estrellas", "3 Estrellas", "4 Estrellas", "5 Estrellas"]
            if "Precios:" in entries:
                entries["Precios:"]['values'] = ["Bajo", "Medio", "Alto"]
            if "Categorías:" in entries:
                try:
                    categories = self.model.get_categories()
                    entries["Categorías:"]['values'] = categories
                except Exception as e:
            ...
            if supplier_id:
                ...
                if "manage_btn" in entries:
                    entries["manage_btn"].configure(
                        command=lambda: self.manage_supplier_products(supplier_id)
                    )

            # Conectar el botón guardar
            save_btn.configure(command=lambda: self.save_supplier(entries, form_window, supplier_id))
        ...
        try:
            ...
            data = (
                ...
                int(entries["Valoración:"].get().split()[0]
                    ) if entries["Valoración:"].get() else None,
    ...
    def manage_supplier_products(self, supplier_id):
        ...
        if not supplier_id:
            messagebox.showwarning("Advertencia", "Primero debe guardar el proveedor para gestionar productos")
            return
        
        try:
            # Obtener datos del modelo
            supplier_data = self.model.get_supplier_by_id(supplier_id)
            supplier_name = supplier_data[0]
            ...
            available_products_formatted = [f"{p[1]} ({p[2]})" for p in available_products]
            current_products_formatted = [(p[0], p[1]) for p in current_products]
            ...
            # Configurar botones en el Controller
            ...
            messagebox.showerror("Error", f"No se pudo abrir la gestión de productos: {e}")
    ...
    def _refresh_products_ui(self, supplier_id, product_combo, tree):
        ...
        try:
            ...
            # Actualizar combobox
            product_combo['values'] = [f"{p[1]} ({p[2]})" for p in available_products]
            
            # Actualizar treeview
            self.view.refresh_products_tree(tree, [(p[0], p[1]) for p in current_products])
